...
export default function Details() {
  ...
  useEffect(() => {
    // 상세보기 화면에 들어오면 id를 기준으로
    async function getFoodDetail() {
      // 음식 레시피 상세정보 받아오기
      // async 비동기를 썼으니까 await으로 완료될때까지 대기시키기가 가능
  ...
  }, []) // useEffect가 처음 켜졌을때만 동작하게끔 []를 넣어준다 ([]안 넣으면 바뀔때마다 update 발동)
      ...
      <div className='content-container'>
        ...
        <div>
          <button onClick={() => {hAddToFavorite(foodDetailData?.recipe)}} className='favorites-btn'>
            {
              /* 해당 아이디가 favoritesList에 없으면 '즐겨찾기에 추가', 없으면 '즐겨찾기에서 제거' */
          ...
          <ul className='recipe-content'>
            {
              ...
              foodDetailData?.recipe?.ingredients.map((ingredients, idx) => {
                return (
                  // jsx
                  <li key={idx}>
